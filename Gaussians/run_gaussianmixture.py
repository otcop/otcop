...
def main():
    ...
    lam = 1
    print(lr)
    if not os.path.exists('graph_SP_%s'%opt.data):
        os.makedirs('graph_SP_%s'%opt.data)    

    netG = NormalizingFlow(d,flow_length).to(device)
    optimizerG = torch.optim.RMSprop(netG.parameters(), lr=lr, momentum = 0.9)
    losses, kl_losses, w_losses = train_SP(netG,optimizerG,data_loader,base_dist,target_dist,lam,alpha_lam=1.2,lr = lr, num_epochs = 100,ot=True)
    ...
    plotresult(netG,kl_losses,w_losses,base_dist,target_dist,lam,'SP_no','graph_SP_no_%s'%opt.data)
    printresult(netG,base_dist,target_dist,'SP_no','graph_SP_no_%s'%opt.data)
    ...
    plotresult(netG,lam,SP='AL')
    plotresult(netG,kl_losses,w_losses,base_dist,target_dist,lam,'AL','graph_AL')
    printresult(netG,base_dist,target_dist,'AL','graph_AL_%s'%opt.data)
    ...
    rho = 100
    print(lamsize)
    losses, kl_losses, w_losses, kl_1,w_1 = train_ADMM(netG1, netG2, optimizerG1, optimizerG2,data_loader, base_dist,target_dist,lamsize,rho,lr,  num_epochs = 100)
    # plotresult(netG2,lam,SP='ADMM')
    plotresult_ADMM(netG1,kl_losses,w_losses,kl_1,w_1,base_dist,target_dist,lam,'ADMM','graph_ADMM_%s'%opt.data)
    printresult(netG1,base_dist,target_dist,'ADMM','graph_ADMM_%s'%opt.data)
...
def distribution_generator(data):
    if data == '2DGaussian':
        ...
        target_dist = torch.distributions.MultivariateNormal(torch.ones(d).to(device),torch.eye(d).to(device))
        return d, base_dist, target_dist
...
def printresult(netG,base_dist,target_dist,method,folder):
    x = base_dist.sample((10000,))
    y,_ = netG(x)
    yy = target_dist.sample((10000,))
